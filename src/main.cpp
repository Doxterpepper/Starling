...
searchMusic(const std::filesystem::path &musicDir) {
  //
  // This doesn't scale. It will need to be cached eventually if the user has an
  // absurd number of songs or searches their whole file system for songs.
  // That's not even counting the fact that we'll want to load metadata for each
  // song.
  //
...
int main(int argc, char **argv) {
  ...
  // through and has no real logic. It would just setup the playback manager for
  // the application while the backend can be mocked for testing.
  //
  ...
  // files. It may be helpful to have this again. Maybe propper argument parsing
  ...
  //
  // The two longest operations are loading the file and creating a SoundPlayer.
  // The exact reason for each one taking so long is not determined yet.
  // Intuitively creating the file object will take time because it depends on
  // disk reads. The creation of the SoundPlayer requires setting up pulse
  // audio. This very well could be an expensive operation.
  //
  ...
  //
  // The best timing I have is for the creation of a SoundPlayer. This takes on
  // the order of 24.8 ms on my machine. The bulk of the turnaround time was
  // from this operation.
  //
  // Loading a file into memory takes around 20-50μs. This is fast, but without
  // it it can take around 1-2μs to switch to the next song. There is no
  // guarantee it will always take this long. For one test file I saw load times
  // on the order of 20ms.
  //
  /*
  ...
  " microseconds" << std::endl; playback_manager.queue(std::move(sound_file));
  }
