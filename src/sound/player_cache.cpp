...
SoundPlayer *PlayerCache::get_player(const SoundFile *soundfile) {
    ...
    //
    // I'd like a better method to do this, but for now we just track what each
    // previous song settings were and recreate the SoundPlayer if it needs new
    // settings. It's a little awkward, but it works.
    //
    // This is done for performance reasons. The goal is to make the transitions
    // from one song to the next as seamless as possible. So only do this work
    // when absolutely necessary. It takes on the order of 24ms to create a
    // SoundPlayer. That's a noticeable gab in the sound.
    //
    int settings = sound_file_settings(soundfile);
    if (current_player_settings != settings) {
        //
        // This is an expensive operation. Track the time spent doing this for
        // awareness.
        //
