...
public:
  ...
  size_t current_time() const override {
    ...
    //
    // Remember, the data_block_offset is the offset to the string "data". It is
    // followed by 4 bytes representing the length of the data. So the beginning
    // of the sound data is that data string + 4 bytes.
    //
    // To get the current time index, take the current position in the file,
    // subtract the headers (everything up to the "data" string and the length.)
    // and then divide by the number of bytes that represent 1 second of time.
    //
  ...
  void seek_song(size_t offset_seconds) override {
    //
    // The end of the header/metadata is marked by the "data" string and then 4
    // bytes representing the length of the data section. Find that position in
    // the file, then go the provided seconds forward.
  ...
  size_t read_sound_chunk(uint8_t *buffer, size_t buffer_size) override {
    ...
    if (current_position > data_length + data_block_offset + 8) {
      std::cout << "Done with audio data. Not playing the rest of the file."
                << std::endl;
      return 0;
    }
...
private:
  ...
  void load_data_tag() {
    ...
    size_t bytes_read =
        fread(data_buffer, sizeof(uint8_t), sizeof(data_buffer), sound_file);
...
private:
  // Master riff chunk.
  //
  // 0x00 : FileTypeBlocID  (4 bytes) : Identifier « RIFF »  (0x52, 0x49, 0x46,
  // 0x46) 0x04 : FileSize        (4 bytes) : Overall file size minus 8 bytes
  // 0x08 : FileFormatID    (4 bytes) : Format = « WAVE »  (0x57, 0x41, 0x56,
  // 0x45)

  // chunk describing data format;

  // 0x0c : FormatBlocID    (4 bytes) : Identifier « fmt␣ »  (0x66, 0x6D, 0x74,
  // 0x20) 0x10 : BlocSize        (4 bytes) : Chunk size minus 8 bytes, which is
  // 16 bytes here  (0x10) 0x14 : AudioFormat     (2 bytes) : Audio format (1:
  // PCM integer, 3: IEEE 754 float) 0x16 : NbrChannels     (2 bytes) : Number
  // of channels 0x18 : Frequency       (4 bytes) : Sample rate (in hertz) 0x1c
  // : BytePerSec      (4 bytes) : Number of bytes to read per second (Frequency
...
public:
  ...
  std::string file_format_id() const {
    return std::string(reinterpret_cast<const char *>(master_riff_chunk) + 0x08,
                       4);
  }
...
private:
  ...
  // 0x00 : FormatBlocID    (4 bytes) : Identifier « fmt␣ »  (0x66, 0x6D, 0x74,
  // 0x20) 0x04 : BlocSize        (4 bytes) : Chunk size minus 8 bytes, which is
  // 16 bytes here  (0x10) 0x08 : AudioFormat     (2 bytes) : Audio format (1:
  // PCM integer, 3: IEEE 754 float) 0x0a : NbrChannels     (2 bytes) : Number
  // of channels 0x0c : Frequency       (4 bytes) : Sample rate (in hertz) 0x10
  // : BytePerSec      (4 bytes) : Number of bytes to read per second (Frequency
