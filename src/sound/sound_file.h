...
inline long file_search(FILE *binary_file,
    ...
    do {
        read_bytes = fread(search_buffer.data(), sizeof(search_buffer[0]),
                           search_buffer.size(), binary_file);

        for (size_t search_index = 0; search_index < read_bytes;
             search_index++) {
            if (pattern[pattern_index] == search_buffer[search_index]) {
                ...
                if (pattern_index >= pattern.size()) {
                    ...
                    return static_cast<long>(current_position - read_bytes) +
                           (static_cast<long>(search_index + 1) -
                            static_cast<long>(pattern_index));
                }
            } else {
                pattern_index = 0;
            }
...
inline long file_search(const std::filesystem::path &file_path,
                        const std::vector<uint8_t> &pattern) {
...
class WavFile2 : public SoundFile {
  public:
    ...
    size_t current_time() const override {
        ...
        // second of time.
        //
        size_t current_position_sound_data =
            current_offset - (data_block_offset + 4);
    ...
    size_t read_sound_chunk(uint8_t *buffer, size_t buffer_size) override {
        ...
        if (current_position > data_length + data_block_offset + 8) {
            std::cout
                << "Done with audio data. Not playing the rest of the file."
                << std::endl;
            return 0;
        }
        int read_bytes =
            fread(buffer, sizeof(uint8_t), buffer_size, sound_file);
        return read_bytes;
    }

    friend std::ostream &operator<<(std::ostream &os,
                                    const WavFile2 &wave_header) {
        os << "File Type Block ID : " << wave_header.file_Type_bloc_id()
           << std::endl;
        ...
        os << "Bytes Per Block : " << wave_header.bytes_per_block()
           << std::endl;
        os << "Bits Per Sample : " << wave_header.bits_per_sample()
           << std::endl;
        os << std::endl;
        return os;
    }
  ...
  private:
    void load_header() {
        ...
        size_t read_bytes =
            fread(header, sizeof(header[0]), sizeof(header), sound_file);

        if (read_bytes != sizeof(header)) {
            std::cerr << "Expected to read a header of " << sizeof(header)
                      << " bytes but got " << read_bytes << std::endl;
            // raise exception.
        }
    ...
    void load_data_tag() {
        ...
        size_t bytes_read = fread(data_buffer, sizeof(uint8_t),
                                  sizeof(data_buffer), sound_file);
...
class WavFile {
  public:
    std::string file_Type_bloc_id() const {
        return std::string(reinterpret_cast<const char *>(master_riff_chunk),
                           4);
    }
    ...
    std::string file_format_id() const {
        return std::string(
            reinterpret_cast<const char *>(master_riff_chunk) + 0x08, 4);
    }
...
inline std::unique_ptr<SoundFile>
open_sound_file(const std::filesystem::path &file_path) {
