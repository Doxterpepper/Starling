...
TEST_CASE("Read wav header.", "[wav]") {
    ...
    //
    // Note: The time to create a wav file will depend on the speed of the
    // storage device. In my case, I'm seeing times on average of 36
    // microseconds.
    //
    auto creation_duration =
        std::chrono::duration_cast<std::chrono::microseconds>(
            end_wav_creation - start_wav_creation);
    std::cout << "WavFile construction took " << creation_duration.count()
              << " us";
...
TEST_CASE("Read 24-bit wav buffer", "[wav]") {
    ...
    std::vector<uint8_t> expected_buffer = {
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x49, 0x6A, 0x06, 0x49, 0x6A, 0x06,
        0x21, 0xCE, 0x0C, 0x22, 0xCE, 0x0C, 0x14, 0x25, 0x13, 0x13, 0x25, 0x13};
    ...
    // CHECK(std::equal(buffer.begin(), buffer.end(), expected_buffer.begin(),
    // expected_buffer.end()));
...
TEST_CASE("Search binary file 1", "[binary_search]") {
    uint8_t test_data[] = {0x71, 0x12, 0x0b, 0xaf, 0xe1, 0xe2, 0x4d, 0x6c,
    ...
    size_t written_bytes =
        fwrite(test_data, sizeof(test_data[0]), sizeof(test_data), binary_file);
    fclose(binary_file);
...
TEST_CASE("Read 5 seconds of file.", "[wav]") {
    ...
    while (current_reads < max_reads && current_seconds < target_seconds) {
        ...
        size_t read_bytes =
            wav_file.read_sound_chunk(buffer.data(), buffer.size());
